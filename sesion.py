...
app.config['UPLOAD_FOLDER'] = r"C:\Users\luzsa\OneDrive\Documentos\Septimo semestre\Desarrollo de Software\Cup-puntos\venv\uploads"
...
@app.route('/iniciop',methods=['GET','POST'])
def iniciop():
    if not g.user:
        return redirect(url_for('login'))
    return render_template("iniciop.html")
...
def puntos():
    ...
    try:
        ...
        try:
            for data in acumP:
                puntosF = puntosF + data[0]
                print(puntosF)
        except:
            puntosF = 0 
        Modelo.entities(session['user'],'ProfileLoad','Se cargo el perfil del usuario')
        return render_template('puntos.html',puntos=puntos, expiracion=date_,puntosF=puntosF )
...
def historial():
    ...
    try:
        ...
        try:
            ...
            return render_template('puntos.html',errorLog=errorLog)
